...
3. ~~Upstream: inject `report` as a typed MCP tool into the delegate's session~~ — **rejected 2026-08-18**: an embedded MCP server is off-brand for consult's CLI+files, no-daemons design and multiplies the per-agent testing matrix. The designated confined-reporting design, if ever needed, is a CLI-native **mailbox shim**: stage a `consult-report` shim into the sandbox's `jobRoot/bin` (existing staging mechanism), write NDJSON to a writable job-private mailbox file, and let the log-owning worker tail, validate, and append. Reopen MCP only if a shell-less profile (copilot, `--deny-tool=shell`) ever needs reporting.
